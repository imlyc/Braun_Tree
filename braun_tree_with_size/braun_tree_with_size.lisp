...
(defunc stree-leafp (tr)
  :input-contract (streep tr)
  :output-contract (booleanp (stree-leafp tr))
  (if (endp tr)
    t
    nil))

(defthm stree-not-end
  (implies (streep tr)
           (implies (not (endp tr))
                    (and (not (endp (rest tr)))
                         (not (endp (rest (rest tr))))
                         (not (endp (rest (rest (rest tr)))))
                         (endp (rest (rest (rest (rest tr)))))))))
...
(defunc stree-leaf-c ()
  :input-contract t
  :output-contract (streep (stree-leaf-c))
  nil)
...
(defunc stree-size-it (tr)
  ...
  (if (endp tr)
    0
    (+ 1 (+ (stree-size-it (stree-left tr)) (stree-size-it (stree-right tr))))))

(defunc stree-c (data lc rc)
  :input-contract (and (streep lc) (streep rc))
  :output-contract (streep (stree-c data lc rc))
